...
- local panel over 120 MiB: `400` before CSV loading;
...
- no eligible event for automatic audit selection: `422` with a domain explanation.
...
whitespace-only identifiers are fatal before event detection or forecasting. Missing, negative,
or non-finite required units are also fatal. Duplicate grain is checked after identifier and date
normalization, and column names that collide after trimming are rejected before analysis.
